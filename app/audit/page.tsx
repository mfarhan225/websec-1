...
function Toast({ msg, onClose }: { msg: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed right-4 top-4 z-50 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-bg)] px-3 py-2 text-sm text-primary shadow">
      {msg}
    </div>
  );
}
...
export default function Audit() {
  ...
  const suspicious = useMemo(() => {
    ...
    const brute = [...failsByIp.entries()]
      .filter(([_, c]) => c >= 3)
      .map(([ip, c]) => ({ type: "Brute force?", ip, count: c }));
        ...
        <div className="md:col-span-2 surface p-4 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
          <div className="max-h-[28rem] overflow-auto rounded-lg">
            <table className="w-full text-left text-sm">
              <thead className="sticky top-0 z-10 bg-[var(--surface-bg)] text-muted">
                <tr>
                ...
                {view.map((r) => (
                  <tr
                    ...
                    className={[
                      ...
                      r.ok ? "" : "bg-red-50/40 dark:bg-red-500/5"
                    ].join(" ")}
                  >
            ...
            <div className="mt-2 space-y-2">
              {suspicious.brute.map((b, i) => (
                <div key={`b${i}`} className="rounded-lg border border-red-300/60 dark:border-red-400/40 p-2 text-sm">
                  <div className="font-medium text-red-600 dark:text-red-300">Brute force?</div>
...
function Sparkline({ data }: { data: number[] }) {
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}
         className="rounded-md border border-[var(--surface-border)] bg-white/70 dark:bg-white/10">
